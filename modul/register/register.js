  ...
  $("#error").fadeTo("fast", 0, function() {
    ...
    if (password.length < 8) {
      if (password.length == 0) {
        ...
        error =
          error + "Das Passwort muss mindestens 8 Zeichen enthalten.<br/>";
      }
    } else {
      if (
        ...
        error =
          error +
          "Das Passwort muss aus Gross- sowie Kleinbuchstaben, Zahlen und Sonderzeichen bestehen.<br/>";
      } else {
        ...
        success: function(data) {
          if (data) {
            ...
            $("#pageContent").fadeOut("slow", function() {
              $("#pageContent").html(
                '<div class="alert alert-success"><strong>Registriert!</strong> Sie können sich nun mit dem Benutzernamen "' +
                  username +
                  '" <a id="gotoLogin" style="cursor: pointer;" onclick="goToLogin()" class="alert-link">einloggen</a></div>'
              );
